...
export const DEFAULT_WATER_TARIFF: WaterTariffConfig = {
  concessionaria: 'Sabesp SP (Padrão)',
...
const INITIAL_WATER_READINGS: WaterReading[] = [
  {
    ...
    date: '2026-06-25',
    meterReadingM3: 312,
    consumptionPeriodM3: 14,
    costAmountReais: 158.90,
    notes: 'Ciclo Junho',
  },
  {
    id: 'wat-2',
    date: '2026-07-25',
    meterReadingM3: 328,
    consumptionPeriodM3: 16,
    costAmountReais: 176.20,
    notes: 'Ciclo Julho (Movimento alto)',
  },
  {
    id: 'wat-3',
    date: '2026-08-25',
    meterReadingM3: 343,
    consumptionPeriodM3: 15,
    costAmountReais: 167.50,
    notes: 'Ciclo Agosto',
  },
  {
    id: 'wat-4',
...
export const WaterConsumptionManager: React.FC = () => {
  ...
  // 3. Simulador & Previsão da Conta com Hidrômetro Atual
  const [currentMeterInput, setCurrentMeterInput] = useState<string>('');
  const [dayOfCycle, setDayOfCycle] = useState<number>(15);
  ...
  const handleSaveSimulatedReading = () => {
    ...
    const newRecord: WaterReading = {
      ...
      notes: `Previsão dia ${dayOfCycle}/30 (${forecastData.projectedM3Total} m³ projetados)`,
    };
          ...
          <div className={`p-4 rounded-xl border space-y-3 ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-500 mt-1">
                  Custo por lavagem: <strong className="text-cyan-300 font-mono">R$ {costPerService.toFixed(2)}</strong> ({tariff.waterPerServiceLiters}L)
                </p>
              </div>
            </div>
        ...
        <div className={`p-4 rounded-xl border space-y-4 animate-in fade-in duration-150 ${
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase mb-1.5 text-slate-400">
              Companhia / Modelo Pré-definido:
            </label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
              {[
                { name: 'Sabesp SP', rate: 4.80, sewage: 80, fixed: 38.0 },
                { name: 'Sanepar PR', rate: 4.50, sewage: 80, fixed: 34.0 },
                { name: 'Copasa MG', rate: 5.10, sewage: 90, fixed: 42.0 },
                { name: 'Cedae RJ', rate: 5.40, sewage: 100, fixed: 45.0 },
              ].map((c) => (
                <button
                  key={c.name}
                  type="button"
                  onClick={() => {
                    handleSaveTariff({
                      ...
                      waterRateM3: c.rate,
                      sewagePercent: c.sewage,
                      fixedAvailability: c.fixed,
                    });
                  }}
