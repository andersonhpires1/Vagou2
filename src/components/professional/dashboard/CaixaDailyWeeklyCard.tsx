...
export const CaixaDailyWeeklyCard: React.FC<CaixaDailyWeeklyCardProps> = ({
  ...
  const todayCashData = useMemo(() => {
    ...
    // Fallback realista se base vazia no mock
  ...
  const weekCashData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      // Se histórico vazio no dia passado, alimentar com média realista
      if (dayCount === 0 && i > 0) {
        const simData = [160, 210, 185, 240, 275, 310, 190];
        dayAmount = simData[(d.getDay() + 6) % 7];
        dayCount = Math.round(dayAmount / 45);
